...
public class Board {
    ...
    public int manhattan() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                number++;
                if (tiles[i][j] == 0 || tiles[i][j] == number) continue;

                int[] coo = getShouldBe(tiles[i][j]);

                int value;
                if      (coo[0] == i)   value = Math.abs(coo[1] - j);
                else if (coo[1] == j)   value = Math.abs(coo[0] - i);
                else                    value = Math.abs(coo[0] - i) + Math.abs(coo[1] - j);
    ...
    public boolean equals(Object y) {
        ...
        for (int i = 0; i < this.tiles.length; i++) {
            for (int i2 = 0; i2 < this.tiles.length; i2++) {
                if (this.tiles[i][i2] != that.tiles[i][i2]) return false;
            }
        }
        return true;
    }
    ...
    public Board twin() {
        ...
        int numberX;
        if (coo[0] < n - 1) numberX = tiles[coo[0] + 1][coo[1]];
        else                numberX = tiles[coo[0] - 1][coo[1]];
    ...
    private Stack<Board> getTwinsbyNumber(int number) {
        ...
        if  (col > 0)       twinsByNumber.push(copyChange(coo, Direction.left, number, tiles));
        if  (col < n - 1)   twinsByNumber.push(copyChange(coo, Direction.right, number, tiles));
        if  (row > 0)       twinsByNumber.push(copyChange(coo, Direction.up, number, tiles));
        if  (row < n - 1)   twinsByNumber.push(copyChange(coo, Direction.down, number, tiles));
    ...
    private static void checkNice(int[][] tiles) {
        ...
        for (int i = 0; i < tiles.length; i++) {
            for (int j = 0; j < tiles.length; j++) {
                if (tiles[i][j] < 0) throw new IllegalArgumentException();
            }
        }
    }
